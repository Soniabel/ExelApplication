...
namespace ExelApplication
{
    ...
    public class Parser
    {
		...
		void EvalExp1(out double result )
		{
			...
			{
				GetToken();
				ExpPlusMinus(out partialResult);
				switch (op)
				{
					case "<=":
		...
		void ExpPlusMinus(out double result)
		{
			...
			ExpMultidiv(out result);
			while ((op = token) == "+" || op == "-")
			{
				GetToken();
				ExpMultidiv(out partialResult);
				switch (op)
				{
					case "-":
		...
		void ExpMultidiv(out double result)
		{
			string op;
		...
		void SyntaxErr(Errors error)
		{
			string[] err = {
				...
				"Посилання на неіснучу клітинку"
			};
